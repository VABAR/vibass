...
the parameter is bounded, a uniform distribution in the interval $[0,1]$ will
...
$$
ESS = \frac{(\sum_{i=1}^n w_i)^2}{\sum_{i=1}^n w^2_i}.
$$
...
values are positive (because $\theta$ cannot take negative values) and that
...
n_simulations <- 10000
set.seed(12)
theta_sim <- rgamma(n_simulations,5,0.1)
loglik_pois <- sapply(theta_sim, function(THETA) {
  sum(dpois(data$Us, THETA, log = TRUE))
})
log_ww <- loglik_pois + dgamma(theta_sim, 0.01, 0.01, log = TRUE) - dgamma(theta_sim, 5, 0.1, log=TRUE)
log_ww <- log_ww - max(log_ww)
ww <- exp(log_ww)
ww <- ww / sum(ww)
```
...
n_simulations <- 10000
set.seed(12)
theta_sim <- rgamma(n_simulations, sum(data$Us) + 0.01, length(data$Us) + 0.01)
loglik_pois <- sapply(theta_sim, function(THETA) {
  sum(dpois(data$Us, THETA, log = TRUE))
})
log_ww <- loglik_pois + dgamma(theta_sim, 0.01, 0.01, log = TRUE) - dgamma(theta_sim, sum(data$Us) + 0.01, length(data$Us) + 0.01, log=TRUE)
log_ww <- log_ww - max(log_ww)
ww <- exp(log_ww)
ww <- ww / sum(ww)
```
