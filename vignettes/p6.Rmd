...
$\beta_2$ contain zero?
    ...
    results2 <- MCMCpoisson(formula = y ~ logDose10 + Dose,
                            data = salmonella,
...
points?
    ...
    beta.1 <- summary(results2)[[1]]["logDose10","Mean"]
    ...
    y.pred.matrix <- apply(x.pred.matrix,1,function(x){results2[,"(Intercept)"]+results2[,"logDose10"]*log(x+10)+results2[,"Dose"]*x})
...
compare?
    ...
    glm.output <- glm(formula = y ~ logDose10 + Dose,
      data = salmonella,
      family = "poisson")
    # Summarise output
    summary(glm.output)
