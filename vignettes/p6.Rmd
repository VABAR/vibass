...
and the fake/real response variable `typeFAKE`.
...
For this example we will use the `esdcomp` dataset, which is available in the
`faraway` package. This dataset records complaints about emergency room
...
## Fitting Bayesian Poisson Regression Models in BayesX

Again we use `BayesX` to fit this form of Bayesian generalised linear model.
...
The syntax for fitting a Bayesian Poisson Regression Model with one response
variable, three explanatory variables and an offset will be like so:
...
However, as noted above we need to include an offset in this analysis; since
