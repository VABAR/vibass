...
mm_cols <- c(red = "#b11224", yellow = "#fff200", blue = "#2f9fd7", green = "#31ac55", orange = "#f26f22", brown = "#603a34") 
...
![A pile of plain M&M's candies. Photo by Evan-Amos in public domain.](MM.png){width=60%}
...
#   ggplot(aes(x, y)) +
#   geom_point(
#     colour = sample(mm_cols, 1e3, replace = TRUE)
#   ) +
#   coord_fixed() +
#   theme_void()
...
p <-
  ...
  scale_colour_discrete(type = c(mm_cols["red"], "grey")) +
  coord_fixed(xlim = c(-6, 6), ylim = c(0, 5)) +
  theme_void()

ggdraw() +
  draw_image("mm_bag.png", x = -.3, y = 0) +
  draw_plot(p) +
  annotate(
    "text",
    x = .2, y = .1,
...
cap <- "Binomial probability function for two different values of theta and 20 observations."
theta_labels <- 
  tibble(
    theta = c("0.5", "0.2"),
    r = c(12, 6),
...
theta <- seq(0, 1, 0.01)
# prior <- gamma(0.5+0.5)*theta^{-0.5}*(1-theta)^{-0.5}/(gamma(0.5)*gamma(0.5))
prior <- dbeta(theta, 0.5, 0.5)  # or equivalently
plot(theta, prior, type="l", lwd=4, col="dodgerblue", 
...
where $r$ is the __observed__ number of successes in the sample (here the number of red candies in the sample of size $n$). 
In our first experimet, the likelihood function of $\theta$ for the data 
$\mathcal D_1 = \{r_1 = 4 \text{ red candies in } n_1 = 20 \text{ sampled}\}$ is

$$L(\theta \mid \mathcal D) =  \binom{20}{4}\, \theta^4\,(1- \theta)^{16},$$
```{r, fig.align='center', out.width="60%"}
r <- 4
n <- 20
theta <- seq(0, 1, 0.01)
...
In our first experiment, the posterior distribution of the proportion of red candies is
...
The posterior mean and posterior standard deviation of  the proportion of red candies is
 ...
 $$\mbox{E}(\theta \mid \mathcal D)=\frac{4.5}{4.5+16.5}=0.2143, \,\,\,\, \mbox{DT}(\theta \mid \mathcal D)=\sqrt{\frac{4.5 \cdot 16.5}{(4.5+16.5)^2 \cdot (4.5+16.5+1)}}=0.0875.$$
...
Remember that the posterior predictive distribution of the number of successes $Y^\prime$ in a new sample of size $n^{\prime}$ is a beta-binomial distribution Bb$(\alpha, \beta, n^{\prime})$ with parameters from the posterior distribution and the size of the new experiment. Its probability function is
...
x<-c(0:10)
 ...
 plot(x,pred,type="h",xlim=c(0,10),ylim=c(0,0.4),lwd=2,col="purple", ylab="probability", 
      xlab="number red candies")
```

The posterior mean and posterior standard deviation of the number of red candies is this new sample is

 $$\mbox{E}(Y^{\prime} \mid \mathcal D)=  2.1429, \,\,\,  \mbox{DT}(Y^{\prime} \mid \mathcal D)=1.5403.$$
 
# Time to individual work
 
We propose below an individual exercise that pursues that we can consolidate the basic concepts that we have learned in the previous theoretical session and that we have been practising in this session.

You can code the exercise yourself in R using the examples above, or use the provided interactive application.
