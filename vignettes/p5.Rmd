...
With Gibbs Sampling, we typically discard the initial simulations
(the ``burn-in`` period), while the algorithm converges to the actual
joint distribution, thus removing the dependence on the initial set
of parameter values.

As with the other MCMC algorithms, the resulting simulations form a
...
$\beta_1$ is the regresssion slope, and where the
$\epsilon_i$ are independent with
$\epsilon_i\sim N\left(0,\sigma^2\right)$ $\forall i=1,2,\ldots,n$. For
...
\pi(\beta_0) &= N\left(\mu_{\beta_0}, \tau_{\beta_0}\right), \quad\textrm{and} \\
\pi(\beta_1) &= N\left(\mu_{\beta_1}, \tau_{\beta_1}\right)
\end{align*}
$$
...
&\sim N\left((\tau_{\beta_1} + \tau\sum_{i=1}^n x_i^2)^{-1}
...
standard distributions as full conditionals and there are standard functions in R
to obtain the simulations. We shall do this now for an example in ecology, looking
at the relationship between water pollution and mayfly size.
...
\pi(\beta_0) &= N\left(0, 0.0001\right), \quad\textrm{and} \\
\pi(\beta_1) &= N\left(0, 0.0001\right).
\end{align*}
$$
