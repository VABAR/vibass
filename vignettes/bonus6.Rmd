---
title: 'Practical 6: Linear models Solutions'
author: "VIBASS4"
date: "July 2021"
output:
  html_vignette:
    ...
    fig_height: 4
vignette: >
  %\VignetteIndexEntry{Practical 6: Linear models Solutions}
...
This model can be fitted using IS by sampling values for $\theta =(\alpha, \beta,
...
log_ww <- loglik_biv + dnorm(theta_sim[, 1],0,sqrt(1/0.001)) + dnorm(theta_sim[,2], 0, sqrt(1/0.001)) + dgamma(theta_sim[, 3], 0.01, 0.01)
log_ww <- log_ww - dnorm(theta_sim[, 1], 1.71, 10) - dnorm(theta_sim[, 2], 0, 10) - dlnorm(theta_sim[, 3], 0, 1.25)
log_ww <- log_ww - max(log_ww)
ww <- exp(log_ww)
ww <- ww / sum(ww)
...
```{r}
post_mean_alpha <- sum(theta_sim[, 1] * ww)
post_mean_alpha
post_var_alpha <- sum(theta_sim[, 1]^2 * ww)- post_mean_alpha^2
post_var_alpha
post_mean_beta <- sum(theta_sim[, 2] * ww)
post_mean_beta
post_var_beta <- sum(theta_sim[, 2]^2 * ww)- post_mean_beta^2
post_var_beta
...
rq <- function(theta) {
  ...
  res[1] <- rnorm(1, theta[1], sd = sqrt(1 / 10)) #Sample alpha
  res[2] <- rnorm(1, theta[2], sd = sqrt(1 / 10)) #Sample beta
...
#Proposal distribution: log-density
logdq <- function(new.theta, theta) {
  res <- dnorm(new.theta[1], theta[1], sd = sqrt(1 / 10), log = TRUE) #Alpha
  res <- res + dnorm(new.theta[2], theta[2], sd = sqrt(1 / 10), log = TRUE) #Beta
  res <-  res + 
    dlnorm(new.theta[3], meanlog = log(theta[3]), sdlog = sqrt(1 / 10), log = TRUE) #Tau
}

#Prior distribution
logprior <- function(theta) {
  res <- dnorm(theta[1], 0, sd = sqrt(1 / 0.001), log = TRUE) #alpha
  res <- dnorm(theta[2], 0, sd = sqrt(1 / 0.001), log = TRUE) #beta
...
#Beta
plot(theta[, 2], type = "l", main = "MCMC samples",
  ylab = expression(beta))
plot(density(theta[, 2]), main = "Posterior density",
  xlab = expression(beta))
#Precision
plot(theta[, 3], type = "l", main = "MCMC samples", ylab = expression(tau))
plot(density(theta[, 3]), main = "Posterior density", 
  xlab = expression(tau))
```
