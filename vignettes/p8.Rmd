...
$$
\begin{array}{rcl}
y_{ij} \mid \theta & \sim & \text{Bi}(1,\, \theta)\\
\text{logit}(\theta) & = & \beta_0\\
\beta_0 & \sim & \mathcal{N}(0,\, \tau_{\beta_0})
\end{array}
$$

Note that now the probability $\theta$ is not modelled directly but that its logit
...
```{r}
red.bin <- rep(c(1,0), times = c(sum(data$red), sum(data$MMs) - sum(data$red)))
```
...
$\theta = \text{expit}(\beta_0) = 1/(1 + \exp(-\beta_0))$.
...
<!-- This is probably incorrect. Check. -->
<!-- This sentence might have been intended for the Normal model -->
...
logarithm of the mean (i.e., $\lambda$) is modelled on a vector of covariates.
...
$$
\begin{array}{rcl}
y_i \mid \mu,\, \tau& \sim & \mathcal{N}(\mu,\, \tau)\\
\mu & = & \beta_0\\
\beta_0 & \sim & \mathcal{N}(0,\, \tau_{\beta_0})\\
