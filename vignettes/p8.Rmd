...
hierarchical models will be explained. The examples that have been considered
...
In particular, the following software will be considered:
...
time, making multivarite inference with `INLA` can be difficult or impossible.
However, in many cases this is not needed and `INLA` can fit models in a
fraction of what it takes with MCMC. In addition, `BayesX` and `INLA` have a
very simple interface to define models using a `formula` (similarly as it is
done using `glm` and `gam`).
...
The model that we will be using now is slightly different to the one
prevented in previous practicals. In particular, instead of a Binomial we
will consider each one of the Bernouilli trials, so that data is encoded
...
Note that now probability $\theta$ is not modeled directly but that its logit
depends on a linear predictor, which is made of a single intercept term
$\alpha$. This term is assigned a (vague) Gaussian prior.

You can use the data from the proportion of red lacasitos obtained
before:

```{r,tidy = TRUE, eval = FALSE}
require(gsheet)
data <- gsheet2tbl('https://docs.google.com/spreadsheets/d/19xrl9tk_Bpvfq1Os9ZuZ13kBNF4RbhPq-7IwW2KUFBE/edit?usp=sharing')
```

In case you have lost your data, you can use the following:


```{r eval = TRUE}
data <- data.frame(lacasitos = c(35, 36, 35), red = c(5, 8, 9))
```
...
red.bin <- rep(c(1,0), times = c(sum(data$red), 
  sum(data$lacasitos) - sum(data$red)))
```
...
Note that the output shows the summary of the intercept term and not
...
A Poisson model has already been used for the *Game of Thrones* dataset. In this case,
...
In this case the linear predictor is made of a single intercept term, which
...
```{r, tidy = TRUE, eval = FALSE}
require(gsheet)
data <- gsheet2tbl('https://docs.google.com/spreadsheets/d/1x48c2BspJYlIoH1EydQp2Aw0sHBeIrm-20JmpvFV48c/edit?usp=sharing')
Us <- data$Us
```
In case you have lost the data, you can use the following:
...
The last model is based on the height data collected before. The actual
...
In this case, we have explicitely written the mean $\mu$ equal to a linear
...
data <- gsheet2tbl('https://docs.google.com/spreadsheets/d/1_o6y1sgMf2DiFi5MEZ-kGpv161g9JlFihaGDWzKgXdE/edit?usp=sharing')
heights <- data$height
```
...
are important differences between these two packages but, in practice, model
fitting works very similarly.


## Binomial model

First, a model to the lacasitos dataset will be fitted:
...
The summary output shows summary statistics of the intercept, which is not
the proportion. We need to back-transform the output but, in this case, 
we need to back-transform the marginal of the intercept (because there
...
Next, the analysis of the *Game of Thrones* dataset will be considered and 
...
Again, in order to obtain the posterior marginal distribution of mean
$\theta$,
...
Finally, the heights dataset will be considered to fit a Bayesian model
...
As mean $\mu$ is equal to the intercept $\alpha$, there is no need
