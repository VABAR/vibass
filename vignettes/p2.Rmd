...
$\pi(\mbox{logit}(\theta) \mid \mathcal D)$ as individual work at the end of the practice.
...
hist(oddstheta.r, breaks = 100, freq=FALSE, col="gray98", main="", xlim=c(0,1.5), ylim=c(0,4), 
    xlab=expression(paste("odds of the proportion of red candies ", theta )), ylab="posterior") 
```
...
mean(oddstheta.r)
var(oddstheta.r)
sqrt(var(oddstheta.r))
quantile(oddstheta.r, probs = c(0.05, 0.5, 0.95))
quantile(oddstheta.r, probs = c(.025,  0.975))
sum(oddstheta.r>1)/10000
```  
...
var(theta.dif)
sqrt(var(theta.dif))
quantile(theta.dif, probs = c(0.05, 0.5, 0.95))
quantile(theta.dif, probs = c(0.025,  0.975))
sum(theta.dif>0)/10000
``` 
...
1. Compute the posterior distribution of the proportion of red M&M's in the logit scale as well as its main characteristics 
