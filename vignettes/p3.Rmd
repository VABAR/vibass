...
According to Wikipedia: ``*A Game of Thrones* is the first novel in *A Song of Ice and Fire*, a series of fantasy novels by the American author George R. R. Martin. In the novel, recounting events from various points of view, Martin introduces the plot-lines of the noble houses of Westeros, the Wall, and the Targaryens. The novel has inspired several spin-off works, including several games. It is also the namesake and basis for the first season of the television series *Game of Thrones*.''


If you want to get a bit of atmosphere you can go to https://www.youtube.com/watch?v=KPLWWIOCOOQ where you can see a trailer of the series and if you want the music of this series to accompany you throughout the practice you can go to https://www.youtube.com/watch?v=Hf9u3jPvkkI.
...
$$\pi(\lambda)=\frac{ \beta_0^{\alpha_0}}{\Gamma(\alpha_0) }\, \lambda^{(\alpha_0-1)}\,\mbox{e} ^{-\lambda\,\beta_0}, \,\, \lambda>0$$
...
\begin{align*}
\mbox{Prior 1: }& \pi_1(\lambda) \propto\lambda^{-1/2}\\
\mbox{Joan, Prior 2: }& \pi_2(\lambda)=\mbox{Ga}(\alpha_0=105.8, \beta_0=4.6).
\end{align*}
...
$$\mbox{E}(\lambda)=105.8/4.6= 23, \,\,\mbox{DT}{(\lambda)}= \sqrt(105.8/4.6^2)=2.236 \,\,\mbox{letters u}$$
...
=& \prod_{i=1}^{n}\, P(Y_i=y_i \mid \lambda) = \frac{\lambda^{\sum y_i}\mbox{e}^{-n \lambda}}{\prod_{i=1}^{n}\, y_i!}
\end{align*}
...
\begin{align*}
L(\lambda \mid \mathcal D)\propto & \lambda^{643}\mbox{e}^{-25 \lambda}
\end{align*}
...
\begin{align*}
\mbox{Posterior 1: }& \pi_1(\lambda \mid \mathcal D) =\mbox{Ga}(\alpha = 643+0.5= 643.5 , \beta =25 )\\
\mbox{Posterior 2: }& \pi_2(\lambda \mid \mathcal D)=\mbox{Ga}(\alpha = 643+105.8=748.8, \beta =25+4.6=29.6 )
\end{align*}

Next, we plot on the same graph the informative prior distribution $\mbox{Ga}(\alpha_0=105.8, \beta_0=4.6)$ from Joan (in blue) and the two posterior distributions, 
$\pi_1(\lambda \mid \mathcal D) =\mbox{Ga}(\alpha = 643.5, \beta =25 )$ in dark green and $\pi_2(\lambda \mid \mathcal D)=\mbox{Ga}(\alpha = 748.8, \beta =29.6)$ in light green. 
...
\begin{align*}
\mbox{Posterior 1: }& \pi_1(\lambda \mid \mathcal D) =\mbox{Ga}(643.5, 25 ), \,\,\mbox{E} (\lambda \mid \mathcal D)=25.74, \,\,\mbox{DT}(\lambda \mid \mathcal D)=1.0147 \mbox{ letters u} \\
\mbox{Posterior 2: }& \pi_2(\lambda \mid \mathcal D)=\mbox{Ga}(748.8, 29.6 ), \,\,\mbox{E} (\lambda \mid \mathcal D)=25.30, \,\,\mbox{DT}(\lambda \mid \mathcal D)=0.9245 \mbox{ letters u}
\end{align*}
...
\end{align*}
 where E$(Y_{n+1}\mid \mathcal D) = \alpha/\beta$ and Var$(Y_{n+1} \mid \mathcal D) = \frac{\alpha}{\beta} (1+(1/\beta))$.
 
 Suppose now that we randomly select a new page (page 26th) in the novel and want to predict the number of letters u we will find in it. The subsequent posterior predictive distribution is GaPo(643.5, 25, 1) if we work in the framework of the posterior 1 Ga(643.5, 25) or GaPo(748.8, 29.6, 1) if we work in the framework of the posterior 2 Ga(748.8, 29.6). New we represent both predictive distributions
...
\begin{align*}
\mbox{Predictive 1: }& \mbox{GaPo}(643.5, 25, 1 ), \,\, \mbox{E}(Y_{26} \mid \mathcal D)=25.74, \,\,DT(Y_{26} \mid \mathcal{D})=5.17 \mbox{ letters u}\\
\mbox{Predictive 2: }& \mbox{GaPo}(748.8, 29.6, 1), \,\, \mbox{E}(Y_{26} \mid \mathcal D)=25.30, , \,\,DT(Y_{26} \mid \mathcal{D})=5.11 \mbox{ letters u}
\end{align*}
...
\begin{align*}
\mbox{Tauriel, Prior 3: } & \pi_3(\lambda) = \mbox{Ga}(90,3) 
\end{align*}
